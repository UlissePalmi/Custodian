...
**Net worth is recorded daily** (`models/daily_net_worth.py`, `services/history.py`). Only today is directly observable — balances are what the bank says now, holdings are at the latest quote — so the past is reconstructed by walking backwards from today's measured total, removing each day's recorded income/expenses and price movement. Landing exactly on today's figure is a property of that arithmetic, which makes it worth asserting. Two traps the implementation exists to avoid: price movement is weighted by the quantity held the day *before*, because a position appearing is a purchase (cash becoming stock of equal value) rather than a gain; and transfers are ignored entirely since they net to zero. Only *finished* days are stored: today's net worth is still moving, so a row for it would claim to be a closing figure while continuing to change — `read_daily` appends it live instead, the same rule the monthly snapshots follow. `ensure_days` fills every missing day up to yesterday and is called both by the sync timer and on `GET /api/networth/daily`, so opening the app after the Pi was off catches up through the same path. Recording shortly after midnight is exact, since prices are still the previous close and almost nothing has happened yet. Historical prices come from `quotes.history_for` — yfinance for tickers, onvista's `eod_history` for ISINs, which keeps only about a month.

**Two invariants shape the backend.** The monthly ledger is the single source of truth: the yearly table (`backend/app/services/yearly.py`) is aggregated from transactions at request time and never stored, so it cannot disagree with the month views. And nothing aggregate is stored for net worth — the total is recomputed from holdings and account balances in `services/networth.py` on every read, which is why a sync moves the dashboard immediately. History is the one thing written down, a day at a time.
...
Holdings are not pulled on demand the way transactions are: `investments_holdings_get` serves whatever Plaid last collected, so every sync asks for a refresh first or a trade made today is silently absent. The request is a hint rather than a guarantee — Plaid may still answer with the previous view and pick the trade up on the next run.

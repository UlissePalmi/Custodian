...
**Accounts are mapped to real bank accounts** (`models/account.py`, `services/reconcile.py`). `Account.plaid_account_id` binds one to a Plaid account; every sync records what the bank says in `plaid_balance` *without* overwriting `balance`. Both are kept on purpose: `balance` is the ledger's running total, the bank's is the truth, and the gap between them is the only thing that reveals a transaction missed, double-counted or misdated — `GET /api/plaid/reconciliation` lists the gaps and the timer logs them. Two exceptions take the bank's figure directly, because no transaction ever moves them: a brokerage's uninvested cash, and what is owed on a card. Unmapped accounts (a foreign bank) are never touched.

A `credit`-type account holds what is *owed*, stored positive and subtracted from cash at read time — net worth is assets minus debts. A `stocks`-type account's balance is only its uninvested cash; its positions come from `holdings`, and the two add rather than one replacing the other.

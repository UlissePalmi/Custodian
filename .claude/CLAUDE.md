...
A `credit`-type account holds what is *owed*, stored positive and subtracted from cash at read time — net worth is assets minus debts. A `stocks`-type account's balance is only its uninvested cash; its positions come from `holdings`, and the two add rather than one replacing the other. `services/networth.accounts_breakdown` reproduces exactly these rules for `GET /api/accounts/breakdown`, which backs the allocation page — the two views must sum to the same total or one of them is lying.

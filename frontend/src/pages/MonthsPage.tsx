import { Link } from 'react-router-dom'
import { useApi } from '../hooks/useApi'
import { useDataVersion } from '../context/DataVersion'
import { getMonths, type MonthInfo } from '../api'
import { Card } from '../components/ui/Card'
import { Amount } from '../components/ui/Amount'
import { ErrorState, Skeleton } from '../components/ui/States'
import { PageBody, PageHeader } from '../components/ui/PageHeader'
import { MONTH_NAMES, parseMonthKey } from '../utils/months'

function MonthTile({ month }: { month: MonthInfo }) {
  const { year, month: monthNumber } = parseMonthKey(month.monthKey)

  return (
    <Link
      to={`/months/${month.monthKey}`}
      className="flex flex-col rounded-xl border border-terminal-navy/10 bg-white p-4 shadow-sm transition-colors hover:border-terminal-navy/25"
    >
      <span className="text-sm font-semibold text-terminal-navy">{MONTH_NAMES[monthNumber - 1]}</span>
      <span className="text-xs text-slate-500">{year}</span>

      {month.hasData ? (
        <span className="mt-3">
          <Amount value={month.net} signed className="text-sm font-medium" />
          <span className="mt-0.5 block text-xs text-slate-400">net</span>
        </span>
      ) : (
        <span className="mt-3 text-xs text-slate-400">No entries</span>
      )}
    </Link>
  )
}

export default function MonthsPage() {
  const { version } = useDataVersion()
  const { data, loading, error, refetch } = useApi(getMonths, [version])

  const byYear = new Map<number, MonthInfo[]>()
  for (const month of data ?? []) {
    const { year } = parseMonthKey(month.monthKey)
    byYear.set(year, [...(byYear.get(year) ?? []), month])
  }

  return (
    <>
      <PageHeader
        eyebrow="Ledger"
        title="Months"
        subtitle="Pick a month to view and edit its ledger."
      />
      <PageBody>
        {error ? (
          <Card>
            <ErrorState error={error} onRetry={refetch} />
          </Card>
        ) : loading || !data ? (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {Array.from({ length: 12 }).map((_, index) => (
              <Skeleton key={index} className="h-28 w-full rounded-xl" />
            ))}
          </div>
        ) : (
          [...byYear.entries()].map(([year, months]) => (
            <section key={year}>
              <h2 className="mb-3 text-xs font-semibold tracking-wide text-slate-500 uppercase">{year}</h2>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                {months.map((month) => (
                  <MonthTile key={month.monthKey} month={month} />
                ))}
              </div>
            </section>
          ))
        )}
      </PageBody>
    </>
  )
}
